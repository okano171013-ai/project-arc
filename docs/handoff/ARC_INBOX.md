...
**処理済み**：ADR 0074「Notionを当面の個人情報記録の母体とし、
Project ARCは開発継続するが日常使いから一時的に外す」（Owner本人
判断、2026-07-21）。Version41完了後、Owner本人がChatGPT経由で
実際にGrant発行・Memory保存を試みる中、チャットごとの接続不安定さと
Remote MCPのPC依存（PC-off Gap、ADR 0070）が繰り返し障害になり、
「結局使いものになっていない」と判断した。**コードのロールバックは
行っていない**——Version1〜41（665件のテスト合格）はそのまま維持
する。当面Notionを個人情報記録の母体とし、Project ARCは日常使いから
一時的に外れる。将来Remote MCP接続の安定性・PC-off Gapが解決した
