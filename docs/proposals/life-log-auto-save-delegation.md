...
- 食事・栄養：1日1値の集計ではなく、**食事単位**を基本記録とし、
  日次集計は別途生成できる構造にする（例：`MealLog`のような食事
  ごとの新Entity＋日次ロールアップの導出クエリ）。
- 体重：**原則1計測1記録**とし、同日複数計測を許容する（Reflectionの
  ような1日1行の構造には収まらない——専用の時系列Entityが必要）。
- 収入・支出：**取引単位**を基本とする（`Reflection.expenseYen`の
  ような日次合計フィールドでは表現できない——個別取引を記録する
  専用Entityが必要）。

この3点はいずれも「Reflectionを拡張するだけでは済まない」ことを
意味する——Phase2は5章で想定した「Reflectionへのフィールド追加」
ではなく、**新規の時系列Entity（食事ログ・体重ログ・取引ログ）を
それぞれ設計する**規模の作業になる。次Version着手時に、この粒度
要件を前提に設計し直すこと。
