...
import { PROPOSAL_TYPES } from './proposalSchema.js';
...
export function registerProposalCreateTool(server: McpServer, connector: Connector): void {
  server.registerTool(
    'proposal_create',
    {
      ...
      description:
        '書き込み提案（Proposal）を組み立てて返すだけで、何も保存しない。保存するには、この戻り値をそのままproposal_approveへ渡し、Ownerの承認を得る必要がある。',
      inputSchema: {
    ...
    async ({ type, target, payload, reason }) =>
      runTool(() => connector.createProposal({ type, target, payload, reason })),
  );
}
